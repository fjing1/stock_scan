...
COMPLETED_20D_DIR = HISTORY_DIR / "completed_20d"
...
def _write_raw_signals_sheet(writer, df_all: pd.DataFrame):
    raw_cols = [
        ...
        "rank120", "H4_RSI", "H4_FJ", "H4_0_birth", "H4_1_birth", "extra_info",
    ]
...
def export_signal_dashboard(df_all: pd.DataFrame, run_dt: datetime, export_dir: Path, history_dir: Path):
    """导出一个轻量 HTML dashboard，集中显示当前 BUY/SHORT 信号。"""
    ...
    if df.empty:
        ...
        for _, r in df.iterrows():
            ...
            side_cls = "buy" if side == "BUY" else "short"
            side_txt = "BUY" if side == "BUY" else "SHORT"
            cells = [
...
<div class=\"sub\">Run: {run_time} · BUY = 1D分金低位 + 4H 0出 · SHORT = 1D分金转弱 + 4H 1出</div>
<table>
<thead><tr>
...
def _recent_history_result_map(history_dir: Path, run_dt: datetime, max_days: int) -> dict[str, Path]:
    ...
    """
    keep_dates = set(pd.bdate_range(end=run_dt.date(), periods=max_days).date)
    pat = re.compile(r"^scan_result_(\d{8})_\d{6}\.xlsx$")
    out: dict[str, Path] = {}
...
def _build_followup_sheets(
    ...
    base_cols = ["symbol", "板块", "D0_date", "D0_rule", "D0_close"]
    ...
    for sig_date, rows in out_rows.items():
        if not rows:
            continue
        df_sheet = pd.DataFrame(rows).sort_values("symbol").reset_index(drop=True)
...
def _archive_completed_cycles(
    ...
    for sig_date, df_sheet in sorted(completed_sheets.items()):
        ...
        out_name = f"{date_key}共计20天数据.xlsx" if not archive_prefix else f"{archive_prefix}_{date_key}共计20天数据.xlsx"
    ...
    for _, sdf in sorted(followup_sheets.items()):
        ...
        for _, row in sdf.iterrows():
            ...
            rows.append({
                ...
                "signal_date": d0_date,
                "signal_type": "BUY_followup" if side == "BUY" else "SELL_followup",
                "signal_side": side,
...
def _write_summary_sheet(writer, buy_df: pd.DataFrame, sell_df: pd.DataFrame):
    ...
    write_section("买入跟踪（20交易日内）", buy_df, "买入板块Top5统计（按出现只数）")
    write_section("卖出跟踪（20交易日内）", sell_df, "卖出板块Top5统计（按出现只数）")
    ws.freeze_panes = "A3"
...
def _write_followup_section(
    ...
    ws.cell(row=start_row, column=1, value=title).font = title_font
    header_row = start_row + 1
    section_cols = list(section_df.columns) if not section_df.empty else ["symbol", "板块", "D0_date", "D0_rule", "D0_close", "retrigger_dates"]
    for cidx, col_name in enumerate(section_cols, start=1):
        cell = ws.cell(row=header_row, column=cidx, value=col_name)
        cell.fill = header_fill
        cell.font = header_font
...
def _write_combined_followup_sheet(writer, sheet_name: str, buy_df: pd.DataFrame, sell_df: pd.DataFrame):
    ws = writer.book.create_sheet(sheet_name[:31])
    writer.sheets[sheet_name[:31]] = ws
    row = 1
...
def _normalize_sector_with_code(group_name: str) -> str:
    ...
    g = str(raw or "").strip()
    g = re.sub(r"^\s*\d{1,2}\s*", "", g).strip()
    if not g:
        return "99 未分组"

    code_map = {
        "核心指数映射/超大盘核心": "01 核心指数映射/超大盘核心",
        "市场环境": "01 市场环境",
        "AI超级核心": "02 AI超级核心",
...
def download_daily(symbol, period="1y"):
    ...
    df = yf.download(yf_symbol, period=period, interval="1d", auto_adjust=False)
...
def download_4h(symbol, period="90d"):
    yf_symbol = to_yfinance_symbol(symbol)
    df = yf.download(yf_symbol, period=period, interval="4h", auto_adjust=False)
    df = normalize_yf_df(df)
    if df.empty:
        return None
    return df[["Open", "High", "Low", "Close", "Volume"]].dropna()
...
def scan_one_symbol(sym, name, xl: XunLongIndicator):
    ...
    # ---- GannBox BUY A：新 0 后启动买点 ----
    if "Gann_BUY_A" in df_all.columns:
        recent_buy_a = df_all[df_all["Gann_BUY_A"] & daily_recent_mask]
        for idx, row in recent_buy_a.iterrows():
            rows.append({
                "symbol": sym,
                "name": name,
                "signal_date": idx.date(),
                "signal_type": "Gann_BUY_A_new_zero",
                "signal_side": "BUY",
                "model": "GannBox",
                "close": row["Close"],
                "volume": row["Volume"],
                ...
                "extra_info": (
                    f"Gann0={row.get('Gann_0', np.nan):.2f}; "
                    f"Gann1预览={row.get('Gann_1', np.nan):.2f}; "
                    f"段涨幅={row.get('Gann_gain_pct', np.nan):.2%}; 新0后BUY A"
                ),
            })

    # ---- GannBox SELL：同一段 1 确认 ----
    if "Gann_SELL_1_confirmed" in df_all.columns:
        recent_sell_1 = df_all[df_all["Gann_SELL_1_confirmed"] & daily_recent_mask]
        for idx, row in recent_sell_1.iterrows():
            rows.append({
                "symbol": sym,
                "name": name,
                "signal_date": idx.date(),
                "signal_type": "Gann_SELL_1_confirmed",
                "signal_side": "SELL",
                "model": "GannBox",
                "close": row["Close"],
                "volume": row["Volume"],
                ...
                "extra_info": (
                    f"Gann0={row.get('Gann_0', np.nan):.2f}; "
                    f"Gann1={row.get('Gann_1', np.nan):.2f}; "
                    f"段涨幅={row.get('Gann_gain_pct', np.nan):.2%}; 1确认SELL"
                ),
            })

    # ---- 新策略：1D 分金低位 + 4H 0出 = BUY B ----
    if "BUY_B_D1FJ_low_H4_0birth" in df_all.columns:
        recent_buy_b = df_all[df_all["BUY_B_D1FJ_low_H4_0birth"] & daily_recent_mask]
        for idx, row in recent_buy_b.iterrows():
            rows.append({
                "symbol": sym,
                "name": name,
                "signal_date": idx.date(),
                "signal_type": "BUY_B_D1FJ_low_H4_0birth",
                "signal_side": "BUY",
                "model": "D1FJ_H4Gann",
                "close": row["Close"],
                "volume": row["Volume"],
                ...
                "H4_1_birth": row.get("H4_Gann_1_birth_daily", False),
                "extra_info": (
                    f"1D分金={row.get('FJ_value', np.nan):.2f}; "
                    f"1D_RSI={row.get('RSI', np.nan):.2f}; "
                    f"4H_RSI={row.get('H4_RSI_last', np.nan):.2f}; "
                    f"4H分金={row.get('H4_FJ_last', np.nan):.2f}; 4H 0出"
                ),
            })

    # ---- 新策略：1D 分金转弱 + 4H 1出 = SHORT A ----
    if "SHORT_A_D1FJ_weak_H4_1birth" in df_all.columns:
        recent_short_a = df_all[df_all["SHORT_A_D1FJ_weak_H4_1birth"] & daily_recent_mask]
        for idx, row in recent_short_a.iterrows():
            rows.append({
                "symbol": sym,
                "name": name,
                "signal_date": idx.date(),
                "signal_type": "SHORT_A_D1FJ_weak_H4_1birth",
                "signal_side": "SELL",
                "model": "D1FJ_H4Gann",
                "close": row["Close"],
                "volume": row["Volume"],
                ...
                "extra_info": (
                    ...
                    f"1D_RSI={row.get('RSI', np.nan):.2f}; "
                    f"4H_RSI={row.get('H4_RSI_last', np.nan):.2f}; "
                    f"4H分金={row.get('H4_FJ_last', np.nan):.2f}; 4H 1出做空"
                ),
            })
...
def main():
    ...
    df_run = df_meta_new[df_meta_new["enable"] == 1]
    if df_run.empty:
        print("所有股票 enable=0，没有可扫描的标的。")
        return
    ...
    if not all_rows:
        ...
        catchup_dates = _get_catchup_signal_dates(HISTORY_DIR, run_dt, max_bdays=GANN_LOOKBACK_DAYS)
        ...
        if catchup_dates:
            print("本次补回信号日期：", ", ".join([d.isoformat() for d in catchup_dates]))
    ...
    buy_anchors = _extract_anchor_signals(HISTORY_DIR, df_all, signal_side="BUY")
    followup_sheets, completed_sheets = _build_followup_sheets(
        buy_anchors, now, max_days=TRACK_MAX_DAYS, sector_map=sector_map_all
    )
    sell_anchors = _extract_anchor_signals(HISTORY_DIR, df_all, signal_side="SELL")
    ...
    for dt_key in combined_completed_dates:
        ...
        with pd.ExcelWriter(out_file, engine="openpyxl") as writer:
            _write_combined_followup_sheet(writer, dt_key, buy_completed_df, sell_completed_df)
    ...
    recent_result_map = _recent_history_result_map(HISTORY_DIR, now, TRACK_MAX_DAYS)
    ...
    with pd.ExcelWriter(history_path, engine="openpyxl") as writer:
        ...
        for dt_key in combined_dates:
            ...
            if not buy_sdf.empty or not sell_sdf.empty:
                _write_combined_followup_sheet(writer, dt_key, buy_sdf, sell_sdf)
                continue
            ...
            _write_combined_snapshot_sheet(writer, dt_key, buy_snapshot_df, sell_snapshot_df)
