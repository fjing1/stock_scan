...
<div class=\"sub\">Run: {run_time} · 预警买入=4H BUY A/0出 · 正式买入=日线 BUY A/0出 · 预警卖出=4H 1出 · 正式卖出=日线 1出</div>
<table>
<thead><tr>
...
def _collect_lifecycle_signal_rows(history_dir: Path, current_df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = ["symbol", "name", "板块", "signal_date", "signal_type", "signal_side", "close", "model", "extra_info"]
    for c in needed:
    ...
    out["signal_date"] = pd.to_datetime(out["signal_date"], errors="coerce").dt.date
    out = out[out["signal_type"].isin(["预警买入", "正式买入", "预警卖出", "正式卖出"])]
...
def _build_lifecycle_tables(history_dir: Path, current_df: pd.DataFrame, run_dt: datetime, df_run: pd.DataFrame, min_days: int = 14):
    ...
    for symbol, g in sig.groupby("symbol"):
        ...
        buy_starts = formal_buys if not formal_buys.empty else g[g["signal_type"].isin(["预警买入"])]
        if buy_starts.empty:
            continue
        ...
        for _, b in buy_starts.iterrows():
            ...
            prior_warning_buy = g[(g["signal_type"] == "预警买入") & (g["signal_date"] <= bdate)].tail(1)
            ...
            if not future_formal_sells.empty:
                ...
                sdate = s["signal_date"]
                warning_sell = g[(g["signal_type"] == "预警卖出") & (g["signal_date"] >= bdate) & (g["signal_date"] <= sdate)].tail(1)
                warning_sell_date = warning_sell.iloc[0]["signal_date"] if not warning_sell.empty else np.nan
    ...
    if not history_df.empty:
        history_df = history_df.sort_values(["正式卖出日期", "symbol"], ascending=[False, True]).reset_index(drop=True)
    if not observation_df.empty:
        observation_df = observation_df.sort_values(["买入跟踪起点", "symbol"], ascending=[True, True]).reset_index(drop=True)
    return observation_df, history_df
    ...
    if "Gann_SELL_1_confirmed" in df_all.columns:
        ...
        for idx, row in recent.iterrows():
            append_signal(
                ...
                f"日线1出; Gann0={row.get('Gann_0', np.nan):.2f}; Gann1={row.get('Gann_1', np.nan):.2f}; 段涨幅={row.get('Gann_gain_pct', np.nan):.2%}"
            )
...
def main():
    ...
    observation_df, lifecycle_history_df = _build_lifecycle_tables(HISTORY_DIR, df_all, now, df_run, min_days=TRACK_MAX_DAYS)
    ...
    with pd.ExcelWriter(history_path, engine="openpyxl") as writer:
        ...
        _write_simple_table_sheet(writer, "观察列表", observation_df)
        _write_simple_table_sheet(writer, "历史记录", lifecycle_history_df)
    ...
    print(f"✅ 观察列表：{len(observation_df)} 条")
    print(f"✅ 历史记录：{len(lifecycle_history_df)} 条")
    if archived_files:
